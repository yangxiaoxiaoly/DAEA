...
class LambdaSheduler(nn.Module): #GRL的lambda调度器
...
class AdversarialLoss(nn.Module):
    ...
    # def forward(self, source, if_source):
    #     lamb = 1.0
    #     if self.use_lambda_scheduler:
    #         lamb = self.lambda_scheduler.lamb()
    #         self.lambda_scheduler.step()
    #     source_loss = self.get_adversarial_result(source, if_source, lamb)
    
    #     adv_loss = source_loss
    #     return adv_loss
...
class Discriminator(nn.Module):
    def __init__(self, input_dim=85, hidden_dim=100):
